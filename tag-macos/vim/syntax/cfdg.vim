...
fun! CFDG_compile(...)
	" Build command
	let this_file = shellescape(@%)
	let output_file = shellescape(CFDG_get_output_filename())
	...
	let b:last_output_filename = ''
	let cfdg_output = systemlist(command)
	for line in cfdg_output
		if strpart(line, 0, 23) == "The output file name is"
			let b:last_output_filename = strpart(line, 24)
			break
		endif
	endfor
endf
...
fun! CFDG_display()
	...
	if type == 'png'
		if b:png_viewer == ''
			...
			silent exe '!' . b:png_viewer . ' ' . b:last_output_filename . ' &'
			redraw!
		endif
	elseif type == 'svg'
		if b:svg_viewer == ''
			...
			silent exe '!' . b:svg_viewer . ' ' . b:last_output_filename . ' &'
			redraw!
		endif
	else
